  ...
  callbacks: {
    ...
    jwt: async ({ token, user }) => {
      if (user) {
        ...
        token.name = user.name; // Attach the user name to the JWT
      }
      return token;
    },
    // Add userId and name to the session object
    session: async ({ session, token }) => {
      if (token && session.user) {
        session.user.id = token.userId; // Attach userId from JWT to the session
        session.user.name = token.name; // Attach user name from JWT to the session
      }
      return session;
    },
  },
